...
public class CheolyongMain {
	...
	public static void main(String[] args) {
		 ...
		 * System.out.println("==========================================");
		 * System.out.print("📦 물품 인도에 걸리는 시간을 설정해주세요 (분 단위, 예: 3) : "); int procTime =
		 * sc.nextInt();
		 * System.out.println("==========================================");
		 * 
		 * try { LocalDateTime baseTime = LocalDateTime.of(2026, 5, 1, 9, 30, 0, 0);
		 * CurrentTime.curTime = baseTime;
		 * 
		 * 
		 * FlightDAO flightDAO = new FlightDAO(); AirportManagerDao airportManagerDao =
		 * new AirportManagerDao();
		 * 
		 * FlightService flightService = new FlightService(flightDAO);
		 * AirportManagerService airportManagerService = new
		 * AirportManagerService(airportManagerDao);
		 * 
		 ...
		 * CurrentTime.curTime = baseTime.minusMinutes(39); ps.appendQueue("M33333333",
		 * 3); // 김철용 CurrentTime.curTime = baseTime.minusMinutes(25);
		 * ps.appendQueue("M44444444", 4); // 오블랙 CurrentTime.curTime =
		 * baseTime.minusMinutes(10); ps.appendQueue("M77777777", 7); // 약블랙
		 * CurrentTime.curTime = baseTime.minusMinutes(5); ps.appendQueue("M88888888",
		 * 8); // 중블랙 CurrentTime.curTime = baseTime.minusMinutes(2);
		 * ps.appendQueue("M99999999", 9); // 강부자 CurrentTime.curTime =
		 * baseTime.minusMinutes(1); ps.appendQueue("M55555555", 5); // 최골드
		 ...
		 * // ========================================================== // 🎬 [시나리오 2]
		 * 이급박 10분 노쇼 타임아웃 // ==========================================================
		 * System.out.println("\n--- 🎬 [시나리오 2] 이급박 10분 노쇼 대기 ---"); for (int i = 0; i
		 * < 10; i++) { ps.passTime(); } // 09:40에 이급박 호출 취소 → pop → handleStarvation에서
		 * 김철용(49분) Starvation 가로채기
		 * 
		 * // ========================================================== // 🎬 [시나리오 3]
		 * 김철용 에이징(Starvation) 자동 호출 //
		 * ==========================================================
		 * System.out.println("\n--- 🎬 [시나리오 3] 노쇼 후 김철용 에이징 자동 호출 ---");
		 * ps.processPickUp("M33333333", 3, procTime); // 김철용
		 * 
		 * // 강부자(PRESTIGE) 자동 호출 → 처리 ps.processPickUp("M99999999", 9, procTime); //
		 * 강부자
		 * 
		 * // ========================================================== // 🎬 [시나리오 4]
		 * 오블랙 에이징(Starvation) 가로채기 //
		 * ==========================================================
		 * System.out.println("\n--- 🎬 [시나리오 4] 오블랙 에이징 가로채기 ---");
		 * ps.processPickUp("M44444444", 4, procTime); // 오블랙
		 * 
		 * // ========================================================== // 🎬 [시나리오 5]
		 * BQ 평시 룰 순서대로 처리 // BLACK(약블랙→중블랙) → GOLD(최골드) → SILVER(유실버) // ⚠️ 박지각은
		 * NO_SHOW 검증을 위해 처리하지 않음! //
		 * ==========================================================
		 ...
		 * ps.processPickUp("M66666666", 6, procTime); // 유실버 // → 유실버 처리 완료 후
		 * tryCallNextCustomer() → 박지각이 currentTicket으로 자동 호출됨
		 * 
		 * // ========================================================== // 🎬 [시나리오 6]
		 * NO_SHOW 자동 처리 검증 //
		 * ==========================================================
		 * System.out.println(
		 * "\n==========================================================");
		 * System.out.println("🎬 [시나리오 6] NO_SHOW 자동 처리 검증"); System.out.println(
		 * "==========================================================");
		 ...
		 * ); System.out.println("     자동으로 NO_SHOW 처리해야 합니다."); System.out.println();
		 * 
		 * System.out.println("📋 [ 점프 전 상태 ]"); System.out.println("   현재 시각: " +
		 * CurrentTime.curTime.toLocalTime());
		 * System.out.println("   currentTicket: 박지각 (OZ1015, 출국 14:00)");
		 * System.out.println("   대기열 잔여:"); ps.printCurrentQueue(); // 비어있어야 정상 (박지각은
		 * currentTicket이므로 큐에 없음)
		 * 
		 ...
		 * System.out.println("   대기열 잔여:"); ps.printCurrentQueue(); // 완전히 비어있어야 함
		 * 
		 * System.out.println(
		 * "\n==========================================================");
		...
		// 필요한 객체들 초기화
		List<BrandSystem> brands = new ArrayList<BrandSystem>();
		brands.add(new BrandSystem("발렌시아가"));
		DutyFlowSystem dfs = new DutyFlowSystem(brands);

		PickUpSystem pus = new PickUpSystem(new AirportManagerService(new AirportManagerDao()), new FlightService());
		Member m1 = new Member();
		m1.setMemberId(50010);
		Member m2 = new Member();
		m2.setMemberId(228);

		// 테스트 1: 주문 완료 -> 픽업 리스트
		System.out.println("================================================");
		System.out.println(" [주문 완료 -> 픽업 리스트 조회 테스트 시작]");
		System.out.println("================================================");

		// 임의로 상품 주문 및 결제 완료된 주문1, 2를 DB에 삽입
		// PickUp 테이블에 삽입됐는지 확인 (직접 확인)
		// PickUpSystem의 getAllPickUpList()로 조회되는지 확인
		

		// 테스트 2 & 3: 인도(= 픽업) 완료 & 노쇼 -> Orders 테이블 상태 UPDATE

		// 임의로 PICKUP_RESERVED 주문 두 개 지정, 해당 주문들이 처리될 날짜 MLPQ에서 로드

		// 해당 날짜의 주문1은 발권 후 출국시간까지 인도받으러 오지 않게 함(= 노쇼), 주문2는 번호표 발권 후 실제 인도 처리

		// 주문1: Orders 테이블에서 상태가 "NO_SHOW"로 변경됐는지 확인

		// 주문2: Orders 테이블에서 상태가 "PICKED_UP"으로 변경됐는지 확인
